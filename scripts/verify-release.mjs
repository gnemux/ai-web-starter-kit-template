...
await readFile(path.join(root, "scripts/test-foundation-db.mjs"), "utf8");
if (!workflow.includes("node scripts/test-foundation-db.mjs")) throw new Error("CI must execute the fail-closed foundation pgTAP runner");
...
if (rootPackage.scripts?.["test:database"] !== "node scripts/test-foundation-db.mjs") throw new Error("Local and CI database verification must share one fail-closed runner");
if (!rootPackage.scripts?.["test:browser"]?.includes("pnpm build") || !rootPackage.scripts?.["test:browser"]?.includes("test:browser:foundation") || !rootPackage.scripts?.["test:browser:product"]?.includes("pass-with-no-tests")) throw new Error("Browser CI must build first, preserve foundation regression and allow an initially empty product suite");
