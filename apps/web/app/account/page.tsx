...
export default async function AccountPage({ searchParams }: { searchParams: Promise<{ mode?: string; error?: string; message?: string; complete_profile?: string; next?: string }> }) {
  ...
  const completionNext = params.complete_profile === "1" ? normalizeInternalReturn(params.next, productConfig.paths.product) : undefined;
  ...
  return <div className="page narrow">
    ...
    {completionNext ? <Notice>{messages.completeProfileDescription}</Notice> : null}
    <Card><p className="eyebrow">{messages.signedIn}</p><h2>{account.user.email ?? copy.account.title}</h2>
      <ProfileForm initialName={account.profile?.display_name ?? ""} completionNext={completionNext} labels={{ displayName: messages.displayName, displayNameHint: messages.displayNameHint, save: completionNext ? messages.completeProfile : messages.saveProfile, saving: messages.saving, success: messages.profileSaved, error: messages.profileFailed, dismiss: messages.dismissNotification }} />
