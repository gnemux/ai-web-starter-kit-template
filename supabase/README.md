...
This directory contains one independent final-state foundation migration, not research history. Use `supabase start`, `supabase db reset`, and `supabase test db` only against the generated repository's disposable local project. A real shared or production target requires a reviewed migration plan and explicit approval.
